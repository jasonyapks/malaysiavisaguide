...
| `/insights/` | Index of Jason's own authored articles — evergreen, first-person, separate from `/news/` because news is perishable and these are the pages meant to be cited. Launched 2026-07-27. Articles are markdown under `content/insights/<category>/` |
| `/insights/<category>/` | Category index. Five categories: `comparisons`, `by-nationality`, `expat-living`, `perspective`, `how-to`. A category page is created when its first article lands, never in advance |
| `/insights/<category>/<slug>/` | One authored article. Category is in the URL by Jason's decision (2026-07-27) — keyword-bearing path and a free breadcrumb, at the cost of the URL changing if an article is recategorised. One dynamic route over `content/insights/<category>/<slug>.md`; `src/lib/data/insights.ts` is the (empty, and deliberately so) registry of literal `.tsx` articles, and `assertNoCollisions()` fails the build if a file ever claims a path a folder already owns; `Article` + breadcrumb schema |
