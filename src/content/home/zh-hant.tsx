...
export const copy: HomeCopy = {
  hero: {
    ...
    // heading 是品牌口號，不含「簽證」二字；這一行才是頁面的實際說明。
    // 英文用 "Long-Stay Residency"，中文說「長期居留」——這是中文讀者
    // 真正會搜的詞，不是 "Residency" 的直譯。
    subheading: "馬來西亞簽證與長期居留指南 2026",
    ...
    promises: [
      {
        title: "每個數字都註明出處與效力等級",
        body: "多數都能追溯到設定它的政府頁面，並註明我們最後一次查證的日期。若公佈的文件已落後於窗口的實際做法，頁面會照實說明，並註明所依據的是誰的執業經驗。",
      },
      {
  ...
  sources: {
    ...
    prose: (href) => (
      <>
        ...
        <p>
          這裡說「已核對」，意思是：有人打開了這段文字旁邊列出的那份文件，在裡面找到了該項費用、門檻或年限，並記下了日期。那個日期會公佈在對應的計劃指南上。如果某項規定只在新聞發佈會上宣佈過、卻從未寫進官方文件，指南就會照實這麼說，而不是悄悄挑一個讀起來更好看的數字。而如果官方文件所寫的措辭，窗口在實際操作中已經不再照辦，頁面就會公佈實際執行的做法，註明所依據的執業者姓名與日期，而不是去引用一份我們明知已經脫節的文件。你讀到的屬於哪一類，都會寫在答案旁邊，不會讓你去猜。
        </p>
        <p>
