...
export const copy: HomeCopy = {
  hero: {
    eyebrow: "独立运营 · 每个数字都注明出处与日期",
    // 品牌模板 §9：Your Guide. Your Future. Malaysia.
    // 英文原句是三个短句，中文拆成三行会各只有四个字，在 display 字号下显得
    // 单薄；合并成两行，重音仍落在「马来西亚」——那是这句话真正的落点。
    heading: (
      <>
        你的指南，你的未来
        <br />
    ...
    // heading 是品牌口号，不含「签证」二字；这一行才是页面的实际说明。
    // 英文用 "Long-Stay Residency"，中文说「长期居留」——这是中文读者
    // 真正会搜的词，不是 "Residency" 的直译。
    subheading: "马来西亚签证与长期居留指南 2026",
    lead: "独立、清晰、实用的马来西亚签证、居留与长期生活指引。本站的每一个数字，都核对过官方来源。",
    ...
    promises: [
      {
        title: "每个数字都注明出处与效力等级",
        body: "多数都能追溯到设定它的政府页面，并注明我们最后一次查证的日期。若公布的文件已落后于窗口的实际做法，页面会照实说明，并注明所依据的是谁的执业经验。",
      },
      {
  ...
  sources: {
    ...
    prose: (href) => (
      <>
        ...
        <p>
          这里说「已核对」，意思是：有人打开了这段文字旁边列出的那份文件，在里面找到了该项费用、门槛或年限，并记下了日期。那个日期会公布在对应的计划指南上。如果某项规定只在新闻发布会上宣布过、却从未写进官方文件，指南就会照实这么说，而不是悄悄挑一个读起来更好看的数字。而如果官方文件所写的措辞，窗口在实际操作中已经不再照办，页面就会公布实际执行的做法，注明所依据的执业者姓名与日期，而不是去引用一份我们明知已经脱节的文件。你读到的属于哪一类，都会写在答案旁边，不会让你去猜。
        </p>
        <p>
