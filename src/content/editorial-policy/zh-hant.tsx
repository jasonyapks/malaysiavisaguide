...
export const copy: EditorialPolicyCopy = {
  ...
  standfirst: (
    <>
      本站的每一個數字都能追溯到具名的出處，由具名的人複核，並標註最後查證的日期。絕大多數直接來自官方文件。少數則做不到：政府自己公佈的指南，有時會落後於櫃檯上實際執行的規則；遇到這種情況，該數字會被明確標示為它本來的性質，而不是混在其餘數字裡含糊帶過。這一頁說明這套做法怎麼運作，以及規則變動時會怎麼處理
      —— 馬來西亞的簽證規則，變動得很頻繁。
    </>
  ),
  ...
  oneSourceOfTruth: {
    ...
    body: (href) => (
      <>
        ...
        <p>
          這份檔案背後的規則，並不是「每個數字背後都有一份現行的官方文件」，而是
          <strong>沒有一個數字是來路不明的</strong>
          。每個數字都帶有以下三種效力之一，而頁面會告訴你，你讀到的是哪一種：
        </p>
        <ul className="ml-5 list-disc space-y-2">
          <li>
            <strong>官方已公佈</strong> ——
            現行的政府文件載明這個數字，並附有鏈接。本站絕大部分內容屬於這一類。
          </li>
          <li>
            <strong>已宣佈，文件待更新</strong> ——
            主管機關已經改了規則，但其公佈的指南仍顯示舊條款。頁面會附上一則註明日期的提示，說明改了什麼、這個說法出自誰，並明確指出頁面上顯示的是否仍是被取代的舊數字。
          </li>
          <li>
            <strong>實務操作</strong> ——
            官方文件無論正反都沒有公佈這個數字，它依據的是一位具名從業者的說法，並註明日期。MM2H
            的代理費收費表就是最清楚的例子：MOTAC 於 2025 年 12
            月的指南並未公佈它，但它是固定的；讀者若在不知情的情況下比較報價，等於毫無防備。
          </li>
        </ul>
        <p>
          三者都不屬於的數字，不會發布。它會被列進一份公開的待查問題清單，而不是靠猜
          —— 一個自信而錯誤的數字，是本站唯一承擔不起的東西。
        </p>
      </>
    ),
  },
