...
export const copy: EditorialPolicyCopy = {
  ...
  standfirst: (
    <>
      本站的每一个数字都能追溯到具名的出处，由具名的人复核，并标注最后查证的日期。绝大多数直接来自官方文件。少数则做不到：政府自己公布的指南，有时会落后于柜台上实际执行的规则；遇到这种情况，该数字会被明确标示为它本来的性质，而不是混在其余数字里含糊带过。这一页说明这套做法怎么运作，以及规则变动时会怎么处理
      —— 马来西亚的签证规则，变动得很频繁。
    </>
  ),
  ...
  oneSourceOfTruth: {
    ...
    body: (href) => (
      <>
        ...
        <p>
          这份档案背后的规则，并不是「每个数字背后都有一份现行的官方文件」，而是
          <strong>没有一个数字是来路不明的</strong>
          。每个数字都带有以下三种效力之一，而页面会告诉你，你读到的是哪一种：
        </p>
        <ul className="ml-5 list-disc space-y-2">
          <li>
            <strong>官方已公布</strong> ——
            现行的政府文件载明这个数字，并附有链接。本站绝大部分内容属于这一类。
          </li>
          <li>
            <strong>已宣布，文件待更新</strong> ——
            主管机关已经改了规则，但其公布的指南仍显示旧条款。页面会附上一则注明日期的提示，说明改了什么、这个说法出自谁，并明确指出页面上显示的是否仍是被取代的旧数字。
          </li>
          <li>
            <strong>实务操作</strong> ——
            官方文件无论正反都没有公布这个数字，它依据的是一位具名从业者的说法，并注明日期。MM2H
            的代理费收费表就是最清楚的例子：MOTAC 于 2025 年 12
            月的指南并未公布它，但它是固定的；读者若在不知情的情况下比较报价，等于毫无防备。
          </li>
        </ul>
        <p>
          三者都不属于的数字，不会发布。它会被列进一份公开的待查问题清单，而不是靠猜
          —— 一个自信而错误的数字，是本站唯一承担不起的东西。
        </p>
      </>
    ),
  },
